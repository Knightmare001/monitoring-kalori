...
export default function MacroCard({ label, consumed = 0, target = 0, unit = 'g', color = 'emerald' }) {
  ...
  return (
    <div className={`card flex flex-col gap-3 ${isCalori ? 'p-5' : 'p-4'}`}>
      {/* Header: ikon + label + badge persen */}
      ...
      {/* Angka utama — Kalori lebih besar sebagai "bintang" */}
      <div>
        <div className="flex items-end justify-between mb-1.5">
          ...
          <span className="text-xs text-slate-400">
            / {Math.round(target).toLocaleString('id-ID')}{unit}
          </span>
        </div>
      ...
      {/* Status sisa / melebihi */}
      <p className={`text-xs font-medium ${over ? 'text-red-500 dark:text-red-400' : 'text-slate-400 dark:text-slate-500'}`}>
        {over
